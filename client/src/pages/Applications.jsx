            ...
            isEdited ?
              <>
                <label className='flex items-center' htmlFor="resumeUpload">
                  <p className='bg-blue-100 text-blue-600 px-4 py-2 round-lg mr-2 cursor-pointer'> {t('Select Resume')}</p>
                  <input id='resumeUpload' onChange={e => setResume(e.target.files[0])} accept='application/pdf' type="file" hidden />
                  <img src={assets.profile_upload_icon} alt='' />
                </label>
                <button onClick={() => setEdit(false)} className='bg-green-100 border border-green-400 round-lg px-4 py-2'>{t('Save')}</button>
              </>
              :
